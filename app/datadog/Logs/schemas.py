...
class SearchLogsRequest(BaseModel):
    ...
    page_limit: Optional[int] = Field(
        None, description="Pagination limit", alias="page[limit]"
    )
...
class AggregateLogsRequest(BaseModel):
    ...
    query: str = Field(..., description="Search query using Datadog log search syntax")
    ...
    compute: Optional[List[Dict[str, Any]]] = Field(
        None, description="Compute aggregations (e.g., count, cardinality, percentiles)"
    )
    filter_query: Optional[str] = Field(None, description="Additional filter query")
...
class ListLogsRequest(BaseModel):
    ...
    query: str = Field("*", description="Search query (use '*' for all logs)")
    from_time: Optional[int] = Field(
        None,
        description="Start time in milliseconds since epoch (default: 2 hours ago)",
        alias="from",
    ...
    status: Optional[str] = Field(
        None, description="Filter by status (error, warn, info, debug)"
    )
    limit: Optional[int] = Field(
        100,
