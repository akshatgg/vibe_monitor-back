...
class TimeseriesQuery(BaseModel):
    ...
    data_source: str = Field("metrics", description="Data source (default: 'metrics')")
    query: str = Field(
        ..., description="Metrics query string (e.g., 'avg:system.cpu.user{*}')"
    )
    name: Optional[str] = Field(None, description="Name for this query")
...
class QueryTimeseriesRequest(BaseModel):
    ...
    data: Optional[TimeseriesFormulaAndFunction] = Field(
        None, description="Formula and function data for complex queries"
    )

    from_timestamp: int = Field(
        ..., description="Start time in milliseconds since epoch", alias="from"
    )
    to_timestamp: int = Field(
        ..., description="End time in milliseconds since epoch", alias="to"
    )
    ...
    def __init__(self, **data):
        ...
        if self.query and self.data:
            raise ValueError(
                "Provide either 'query' (simple) OR 'data' (complex), not both"
            )
...
class TimeseriesSeries(BaseModel):
    ...
    unit: Optional[List[Optional[Dict[str, Any]]]] = Field(
        None, description="Unit information"
    )
...
class SimpleQueryRequest(BaseModel):
    ...
    to_timestamp: int = Field(..., description="End time in milliseconds since epoch")
...
class EventsSearchRequest(BaseModel):
    ...
    start: int = Field(..., description="Start time in seconds since epoch")
    end: int = Field(..., description="End time in seconds since epoch")
    tags: Optional[str] = Field(
        None, description="Comma-separated list of tags (e.g., 'env:prod,service:api')"
    )
...
class EventItem(BaseModel):
    ...
    date_happened: Optional[int] = Field(
        None, description="Timestamp when event occurred (seconds)"
    )
    alert_type: Optional[str] = Field(
        None, description="Alert type: info, warning, error, success"
    )
...
class TagsListResponse(BaseModel):
    ...
    tags: List[str] = Field(
        ..., description="List of all tags (e.g., ['env:prod', 'service:api'])"
    )
