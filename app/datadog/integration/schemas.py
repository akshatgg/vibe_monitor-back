...
class DatadogIntegrationCreate(BaseModel):
    ...
    app_key: str = Field(..., description="Datadog organization-level Application Key with permissions")
    region: str = Field(..., description="Datadog region (e.g., us1, us3, us5, eu1, ap1, us1-fed)")
...
class DatadogIntegrationStatusResponse(BaseModel):
    ...
    is_connected: bool = Field(..., description="Whether the workspace is connected to Datadog")
    integration: Optional[DatadogIntegrationResponse] = Field(None, description="Integration details if connected")
