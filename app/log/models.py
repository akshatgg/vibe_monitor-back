...
class LogStream(BaseModel):
    ...
    stream: Dict[str, str] = Field(
        description="Stream labels (e.g., {job='app', level='error'})"
    )
...
class LogQueryData(BaseModel):
    ...
    resultType: Literal["streams"] = Field(
        description="Result type (always 'streams' for logs)"
    )
    result: List[LogStream] = Field(description="Log streams")
    stats: Optional[Dict[str, Any]] = Field(
        default=None, description="Query statistics"
    )
...
class LogQueryParams(BaseModel):
    ...
    step: Optional[str] = Field(
        default=None, description="Query resolution step for metric queries"
    )
...
class TimeRange(BaseModel):
    ...
    start: str = Field(
        description="Start time (RFC3339Nano, datetime, or relative like 'now-1h')"
    )
    end: str = Field(
        description="End time (RFC3339Nano, datetime, or relative like 'now')"
    )
