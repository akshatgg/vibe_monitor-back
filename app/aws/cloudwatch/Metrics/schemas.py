...
class DimensionFilter(BaseModel):
    ...
    Value: Optional[str] = Field(None, description="Dimension value (optional for wildcard)")


class MetricInfo(BaseModel):
    """Schema for a single metric"""
    Namespace: Optional[str] = Field(None, description="Metric namespace (e.g., AWS/EC2, AWS/Lambda)")
...
class ListMetricsRequest(BaseModel):
    ...
    Namespace: Optional[str] = Field(None, description="Filter by namespace (e.g., AWS/EC2)")
    MetricName: Optional[str] = Field(None, description="Filter by metric name")
    Dimensions: Optional[List[DimensionFilter]] = Field(None, description="Filter by dimensions")
    Limit: Optional[int] = Field(50, description="Maximum number of metrics to return (default: 50)")
...
class MetricSpecification(BaseModel):
    ...
    Namespace: str = Field(..., description="Metric namespace (e.g., AWS/EC2, VibeMonitor/TestMetrics)")
    MetricName: str = Field(..., description="Metric name (e.g., CPUUtilization, APIResponseTime)")
    Dimensions: Optional[List[MetricDimension]] = Field(None, description="Metric dimensions for filtering")


class MetricStat(BaseModel):
    """Metric stat configuration"""
    Metric: MetricSpecification = Field(..., description="Metric specification with Namespace, MetricName, Dimensions")
    Period: int = Field(..., description="Period in seconds (60, 300, 3600, etc.)")
    Stat: str = Field(..., description="Statistic: Average, Sum, Minimum, Maximum, SampleCount")
...
class MetricDataQuery(BaseModel):
    ...
    metric_stat: Optional[MetricStat] = Field(None, description="Metric stat configuration", alias="MetricStat")


class GetMetricDataRequest(BaseModel):
    """Request schema for getting metric data (time-series)"""
    MetricDataQueries: List[MetricDataQuery] = Field(..., description="List of metric queries")
    ...
    ScanBy: Optional[str] = Field("TimestampDescending", description="Scan order: TimestampAscending or TimestampDescending")
    MaxDatapoints: Optional[int] = Field(50, description="Max datapoints to return (default: 50)")
...
class MetricDataResult(BaseModel):
    ...
    StatusCode: Optional[str] = Field(None, description="Status code: Complete, InternalError, PartialData")
    Messages: Optional[List[Dict[str, str]]] = Field(None, description="Status messages")


class GetMetricDataResponse(BaseModel):
    """Response schema for getting metric data"""
    MetricDataResults: List[MetricDataResult] = Field(..., description="Metric data results")
    Messages: Optional[List[Dict[str, str]]] = Field(None, description="Response messages")
...
class GetMetricStatisticsRequest(BaseModel):
    ...
    Statistics: Optional[List[str]] = Field(None, description="Statistics: Average, Sum, Minimum, Maximum, SampleCount")
    ExtendedStatistics: Optional[List[str]] = Field(None, description="Percentiles: p0.0, p10, p50, p90, p99, p100")
    MaxDatapoints: Optional[int] = Field(50, description="Max datapoints to return (default: 50)")
...
class Datapoint(BaseModel):
    ...
    ExtendedStatistics: Optional[Dict[str, float]] = Field(None, description="Extended statistics (percentiles)")
...
class MetricStreamFilter(BaseModel):
    ...
    MetricNames: Optional[List[str]] = Field(None, description="Metric names within namespace")
...
class MetricStreamInfo(BaseModel):
    ...
    IncludeFilters: Optional[List[MetricStreamFilter]] = Field(None, description="Include filters")
    ExcludeFilters: Optional[List[MetricStreamFilter]] = Field(None, description="Exclude filters")
    StatisticsConfigurations: Optional[List[MetricStreamStatisticsConfiguration]] = Field(None, description="Statistics configurations")


class ListMetricStreamsRequest(BaseModel):
    """Request schema for listing metric streams"""
    Limit: Optional[int] = Field(50, description="Maximum number of streams to return (default: 50)")
...
class AnomalyDetectorConfiguration(BaseModel):
    ...
    ExcludedTimeRanges: Optional[List[Dict[str, datetime]]] = Field(None, description="Excluded time ranges")
...
class DescribeAnomalyDetectorsRequest(BaseModel):
    ...
    Limit: Optional[int] = Field(50, description="Maximum number of detectors to return (default: 50)")
...
class AnomalyDetector(BaseModel):
    ...
    Configuration: Optional[AnomalyDetectorConfiguration] = Field(None, description="Configuration")
    StateValue: Optional[str] = Field(None, description="State: PENDING_TRAINING, TRAINED_INSUFFICIENT_DATA, TRAINED")


class DescribeAnomalyDetectorsResponse(BaseModel):
    """Response schema for anomaly detectors"""
    AnomalyDetectors: List[AnomalyDetector] = Field(..., description="Anomaly detectors")
