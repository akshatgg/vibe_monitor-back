...
class InstantMetricResponse(BaseModel):
    ...
    result_type: str = Field(description="Type of result (vector, matrix, scalar, string)")
...
class TimeRange(BaseModel):
    ...
    end: Union[datetime, str] = Field(description="End time (datetime or relative like 'now')")
    step: Optional[str] = Field(default="60s", description="Query resolution step (e.g., '60s', '5m')")
...
class MetricQueryParams(BaseModel):
    ...
    time_range: Optional[TimeRange] = Field(default=None, description="Time range for range queries")
