...
def prepare_multimodal_messages(
    prompt: str, files: Optional[List[Dict[str, Any]]] = None
) -> List[HumanMessage]:
    """
    ...
    for file_obj in files:
        ...
        if mimetype.startswith("image/"):
            ...
            logger.info(
                f"Added image to multimodal message: {file_obj.get('name', 'unknown')}"
            )
            ...
            logger.info(
                f"Added video to multimodal message: {file_obj.get('name', 'unknown')}"
            )
        ...
        async def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if any(
                        code in error_str for code in ["404", "not found", "401", "403"]
                    ):
                        raise
...
def determine_target_environment(
    user_query: str, environment_context: Dict[str, Any]
) -> Optional[str]:
    """
...
def get_deployed_commit_sha(
    repo_full_name: str, environment_name: str, environment_context: Dict[str, Any]
) -> Optional[str]:
    """
...
async def resolve_default_branch_and_fetch_commits(
    commits_tool: Any, workspace_id: str, repo_name: str, first: int = 5
    ...
    for branch_ref in branch_attempts:
        try:
            ...
            if (
                result
                and "Resource not found" not in str(result)
    ...
    logger.warning(
        f"⚠️ Could not resolve default branch (tried: {', '.join(attempted)})"
    )
...
async def discover_best_matching_repo(
            ...
            if repo_names:
                ...
                for repo in repo_names:
                    ...
                    score = SequenceMatcher(
                        None, service_name.lower(), repo.lower()
                    ).ratio()
                    ...
                    # Bonus for substring matches
                        ...
                        service_name.lower() in repo.lower()
                        or repo.lower() in service_name.lower()
                    ):
                        score += 0.2
                ...
                if (
                    best_match and best_score >= 0.75
                ):  # Increased threshold from 0.5 to 0.75
                    logger.info(
                        f"   Found similar repo: {best_match} (confidence: {best_score:.2f})"
                    )
    ...
    logger.info(
        f"   Fallback: assuming service '{service_name}' has repo '{service_name}'"
    )
...
def parse_tool_error(
    error_str: str, tool_name: str, context: str = ""
) -> Dict[str, Any]:
    """
...
async def llm_decide_next_step(
    ...
    for item in services_investigated:
        ...
        if findings.get("logs_summary"):
            investigated_summary += (
                f"  Logs: {str(findings['logs_summary'])[:200]}...\n"
            )
        if findings.get("metrics_summary"):
            investigated_summary += (
                f"  Metrics: {str(findings['metrics_summary'])[:150]}...\n"
            )
        if findings.get("code_findings"):
            investigated_summary += (
                f"  Code: {str(findings['code_findings'])[:200]}...\n"
            )
        if findings.get("commit_findings"):
            investigated_summary += (
                f"  Commits: {str(findings['commit_findings'])[:200]}...\n"
            )
    ...
    prompt = DECIDE_NEXT_STEP_PROMPT_V1.format(
        ...
        metrics_summary=str(current_findings.get("metrics_summary", "No metrics"))[
            :800
        ],
        code_findings=str(current_findings.get("code_findings", "No code"))[:1200],
        commit_findings=str(current_findings.get("commit_findings", "No commits"))[
            :1000
        ],
    )
    ...
    try:
        ...
        if upstream_services_str and upstream_services_str.upper() != "NONE":
            upstream_services = [
                s.strip() for s in upstream_services_str.split(",") if s.strip()
            ]
...
async def investigate_single_service(
    ...
    if "logs" in tools_to_use:
        ...
        try:
            ...
            if not logs_fetched:
                logger.warning(
                    f"⚠️ No logs tool available or no data returned for {service}"
                )
    ...
    if "metrics" in tools_to_use:
        ...
        try:
            ...
            if (
                "fetch_http_latency_tool" in tools_dict
                or "fetch_cpu_metrics_tool" in tools_dict
            ):
                ...
                if metrics_results:
                    ...
                    findings["metrics_summary"][service] = summarize_metrics(
                        {"raw": combined_metrics}
                    )
            ...
            if not metrics_fetched:
                logger.warning(
                    f"⚠️ No metrics tool available or no data returned for {service}"
                )
        ...
        try:
            ...
            if target_environment:
                deployed_commit_sha = get_deployed_commit_sha(
                    repo_name, target_environment, environment_context
                )
            ...
            if has_tree_tool and has_download_tool:
                ...
                if "get_branch_recent_commits_tool" in tools_dict:
                    ...
                    (
                        ...
                        default_branch,
                        _,
                    ) = await resolve_default_branch_and_fetch_commits(
                ...
                if deployed_commit_sha:
                    ...
                    logger.debug(
                        f"   Using deployed commit SHA: {deployed_commit_sha[:8]}..."
                    )
                elif default_branch:
                    ...
                    logger.warning(
                        "   Could not resolve default branch, trying HEAD (may fail)"
                    )
                ...
                if tree_result and "Error" not in str(tree_result):
                    ...
                    for f in all_files_raw:
                        ...
                        has_config_pattern = any(
                            pattern in f.lower() for pattern in config_patterns
                        )
                    ...
                    files_to_read = (
                        files_to_try  # Use ranking from rank_code_files_by_relevance
                    )

                    logger.info(
                        f"   📖 Reading {len(files_to_read)} files (main files first)"
                    )
                    ...
                    for file_path in files_to_read:
                        try:
                            ...
                            if (
                                code_result
                                and "Error" not in str(code_result)
                                and "not found" not in str(code_result).lower()
                            ):
                                # Analyze the full file (no truncation)
                                summary = await extract_key_info_from_code(
                                    str(code_result), llm
                                )
                            ...
                            logger.warning(
                                f"   Error reading {file_path}: {str(e)[:100]}"
                            )
                            continue
                    ...
                    if code_found and code_summaries:
                        ...
                        logger.info(
                            f"✅ Read {len(files_successfully_read)} files for {service}"
                        )
                elif not code_found:
    ...
    if "commits" in tools_to_use:
        try:
            ...
            if "get_branch_recent_commits_tool" in tools_dict:
                ...
                if commits_result:
                    findings["commit_findings"][service] = summarize_commits(
                        commits_result
                    )
...
async def router_node(state: RCAStateV2, llm: BaseChatModel) -> Dict[str, Any]:
    ...
    try:
        ...
        if content == "casual":
            ...
            logger.warning(
                f"Invalid router response: '{content}'. Falling back to keyword heuristic."
            )
            incident_keywords = [
                "error",
                "fail",
        ...
        intermediate_steps.append(
            {
                "step": "router",
                "result": {"query_type": query_type, "query": query[:100]},
            }
        )
...
async def conversational_node(
    state: RCAStateV2, llm: BaseChatModel, tools_dict: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            intent_response = await asyncio.wait_for(
                llm.ainvoke(messages), timeout=15.0
            )
...
Just ask me a question or describe an issue you're facing!"""
        ...
        if intent == "list_repositories":
            if service_mapping:
                ...
                response = "**Available repositories in your workspace:**\n\n"
        ...
        if intent == "environment_info":
            ...
            logger.info(
                f"Showed environment info: {len(environments)} envs, {len(service_mapping)} services, {sum(len(c) for c in deployed_commits.values())} deployments"
            )
            return {
                "final_report": response,
                "intermediate_steps": state.get("intermediate_steps", []),
            ...
            try:
                if "get_branch_recent_commits_tool" in tools_dict:
                    ...
                    if commits_result:
                        ...
                        logger.info(
                            f"✅ Successfully fetched commits from {successful_branch}"
                        )
                    else:
                        # All branches failed - parse error and provide actionable feedback
                        error_info = parse_tool_error(
                            "", "get_branch_recent_commits_tool", f"repo '{repo_name}'"
                        )
                        response = f"⚠️ {error_info['user_message']}\n\n"
                        response += (
                            f"Tried branches: {', '.join(attempted_branches)}\n\n"
                        )
                        response += "You can view commits by:\n"
                        response += f"• Opening https://github.com/[owner]/{repo_name}/commits\n"
                        response += (
                            "• Running `git log -n 10 --oneline` in your local clone"
                        )
                else:
                    response = "⚠️ Commit fetching tool not available (GitHub integration required).\n\n"
...
async def parse_query_node(state: RCAStateV2, llm: BaseChatModel) -> Dict[str, Any]:
    ...
    try:
        ...
        if service_mapping:
            ...
            available_services += "\n".join(
                [f"- `{service}`" for service in service_mapping.keys()]
            )
...
async def plan_node(state: RCAStateV2) -> Dict[str, Any]:
    ...
    try:
        ...
        if not service_mapping:
            logger.warning(
                "⚠️ No service_repo_mapping provided! Cannot investigate services."
            )
            services_to_check = (
                [primary_service] if primary_service != "unknown" else []
            )
            ...
            logger.info(
                f"Available services in workspace: {list(service_mapping.keys())}"
            )
            ...
            logger.info(
                f"✅ Will check ALL {len(services_to_check)} services to find root cause"
            )
        else:
            # If no primary service identified, check ALL services
            services_to_check = list(service_mapping.keys())
            logger.info(
                f"No specific service mentioned - checking all {len(services_to_check)} services"
            )
        ...
        logger.info(
            f"Plan: Check {len(services_to_check)} services with tools {tools_to_use}"
        )
...
async def investigate_node(
    state: RCAStateV2, tools_dict: Dict[str, Any], llm: BaseChatModel
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for service in services:
                ...
                try:
                    ...
                    if "fetch_logs_tool" in tools_dict:
                        ...
                        if (
                            ...
                            if len(logs_result) > 500:
                                ...
                                logger.info(
                                    f"✅ Logs summary for {service} (Grafana): {summary[:100]}..."
                                )
                            else:
                    ...
                    # Fallback to New Relic
                        ...
                        if logs_result and len(str(logs_result)) > 100:
                            ...
                            logs_summary[service] = summary
                            logger.info(
                                f"✅ Logs summary for {service} (New Relic): {summary[:100]}..."
                            )
                        else:
                            logs_summary[service] = logs_result
                        logs_fetched = True
                    ...
                    # Fallback to Datadog
                        ...
                        if logs_result and len(str(logs_result)) > 100:
                            ...
                            logs_summary[service] = summary
                            logger.info(
                                f"✅ Logs summary for {service} (Datadog): {summary[:100]}..."
                            )
                        else:
                            logs_summary[service] = logs_result
                        logs_fetched = True
                    ...
                    # Fallback to CloudWatch
                        ...
                        if logs_result and len(str(logs_result)) > 100:
                            ...
                            logs_summary[service] = summary
                            logger.info(
                                f"✅ Logs summary for {service} (CloudWatch): {summary[:100]}..."
                            )
                        else:
                            logs_summary[service] = logs_result
                        logs_fetched = True
                    ...
                    if not logs_fetched:
                        logger.warning(
                            f"⚠️ No logs tool available for {service} (need Grafana, New Relic, Datadog, or CloudWatch integration)"
                        )
                        logs_summary[service] = (
                            "No logs tool available - integrate Grafana, New Relic, Datadog, or CloudWatch to collect logs"
                        )
            ...
            if "metrics" in tools_to_use:
                ...
                try:
                    ...
                    if (
                        ...
                        if metrics_results:
                            combined_metrics = "\n".join(metrics_results)
                            metrics_summary[service] = summarize_metrics(
                                {"raw": combined_metrics}
                            )
                            logger.info(
                                f"✅ Metrics for {service} (Grafana): {metrics_summary[service][:100]}..."
                            )
                            metrics_fetched = True
                    ...
                    # Fallback to New Relic
                        ...
                        logger.info(
                            f"✅ Metrics for {service} (New Relic): {metrics_summary[service][:100]}..."
                        )
                        metrics_fetched = True
                    ...
                    # Fallback to Datadog
                        ...
                        metrics_summary[service] = summarize_metrics(
                            {"raw": str(metrics_result)}
                        )
                        logger.info(
                            f"✅ Metrics for {service} (Datadog): {metrics_summary[service][:100]}..."
                        )
                        metrics_fetched = True
                    ...
                    # Fallback to CloudWatch
                        ...
                        logger.info(
                            f"✅ Metrics for {service} (CloudWatch): {metrics_summary[service][:100]}..."
                        )
                        metrics_fetched = True

                    if not metrics_fetched:
                        logger.warning(
                            f"⚠️ No metrics tool available for {service} (need Grafana, New Relic, Datadog, or CloudWatch integration)"
                        )
                        metrics_summary[service] = (
                            "No metrics tool available - integrate Grafana, New Relic, Datadog, or CloudWatch to collect performance metrics"
                        )

                except asyncio.TimeoutError:
                    logger.warning(f"⚠️ Metrics fetch timed out for {service} (30s)")
                    metrics_summary[service] = (
                        "Metrics fetch timed out after 30 seconds"
                    )
                except Exception as e:
                    logger.error(f"Error fetching metrics for {service}: {e}")
            ...
            if "code" in tools_to_use:
                ...
                try:
                    ...
                    if not repo_name:
                        logger.warning(
                            f"⚠️ Service '{service}' not in mapping, trying fallback"
                        )
                        ...
                        if "list_repositories_graphql_tool" in tools_dict:
                            (
                                ...
                                confidence,
                                method,
                            ) = await discover_best_matching_repo(
                            ...
                            logger.info(
                                f"   Auto-discovered repo '{repo_name}' for service '{service}' (confidence: {confidence:.2f})"
                            )
                        else:
                            repo_name = service

                    logger.info(
                        f"📖 Reading code from repository: {repo_name} (service: {service})"
                    )
                    ...
                    if target_environment:
                        ...
                        if "/" not in repo_name and service_mapping:
                            ...
                            for svc, mapped_repo in service_mapping.items():
                                if mapped_repo == repo_name or mapped_repo.endswith(
                                    f"/{repo_name}"
                                ):
                        ...
                        deployed_commit_sha = get_deployed_commit_sha(
                            repo_full_name, target_environment, environment_context
                        )

                        if deployed_commit_sha:
                            logger.info(
                                f"   Using deployed commit SHA: {deployed_commit_sha[:8]}... (environment: {target_environment})"
                            )
                        else:
                            logger.info(
                                f"   No deployed commit found for {repo_full_name} in {target_environment}, using HEAD"
                            )
                    ...
                    if has_tree_tool and (has_download_tool or has_read_tool):
                        ...
                        code_tool = tools_dict.get(
                            "download_file_tool"
                        ) or tools_dict.get("read_repository_file_tool")
                        use_download_tool = has_download_tool

                        # Null check for code_tool
                        if not code_tool:
                            logger.error(
                                f"⚠️ Code tool not found despite has_download_tool={has_download_tool}, has_read_tool={has_read_tool}"
                            )
                            code_findings[service] = "Code reading tool unavailable"
                            continue

                        logger.info(
                            f"   Discovering files in {repo_name} repository..."
                        )
                        ...
                        try:
                            ...
                            if deployed_commit_sha:
                                ...
                                tree_expression = f"{default_branch}:"
                                logger.debug(
                                    f"   Using resolved default branch: {default_branch}"
                                )
                            else:
                            ...
                            if tree_result and "Error" not in str(tree_result):
                                ...
                                for f in all_files_raw:
                                    ...
                                    has_code_ext = any(
                                        f.endswith(ext) for ext in code_extensions
                                    )
                                    has_config_pattern = any(
                                        pattern in f.lower()
                                ...
                                # Use smart ranking helper
                                files_to_try = rank_code_files_by_relevance(
                                    all_files, service
                                )

                                logger.info(
                                    f"   Found {len(all_files)} code/config files, will read ALL of them"
                                )
                                if files_to_try:
                                    logger.info(
                                        f"   Top candidates: {files_to_try[:10]}"
                                    )
                            else:
                                # Fallback: try to find files by scanning (no hardcoded names)
                                logger.warning(
                                    "   Could not get repo tree, will try to discover files"
                                )
                            ...
                            logger.warning(
                                f"   Tree discovery failed: {error_info['user_message']}"
                            )
                            files_to_try = []  # Empty - will be handled by error case
                        ...
                        if code_found and code_summaries:
                            ...
                            logger.warning(
                                f"⚠️ No accessible code files found for {service} (tried: {', '.join(files_tried)})"
                            )
                            code_findings[service] = (
                                f"Repository '{repo_name}' found but couldn't access code files (tried: {', '.join(files_tried[:3])})"
                            )
                    else:
                        logger.warning(
                            "⚠️ No code tools available (need GitHub integration with tree/read capabilities)"
                        )
                        code_findings[service] = (
                            "Code tools not available - GitHub integration required"
                        )

                    # Log environment context usage
                    if target_environment and deployed_commit_sha:
                        logger.info(
                            f"✅ Code read using deployed commit from environment '{target_environment}'"
                        )
                    elif target_environment:
                        logger.warning(
                            f"⚠️ Environment '{target_environment}' specified but no deployed commit found - using HEAD"
                        )

                except Exception as e:
                    error_info = parse_tool_error(
                        str(e), "code_investigation", f"service '{service}'"
                    )
                    logger.error(
                        f"Error reading code for {service}: {error_info['user_message']}"
                    )
            ...
            if "commits" in tools_to_use:
                try:
                    ...
                    if "list_repositories_graphql_tool" in tools_dict:
                        ...
                        if method != "mapping":
                            logger.info(
                                f"   Auto-discovered repo '{repo_name}' for service '{service}' (method: {method}, confidence: {confidence:.2f})"
                            )
                    else:
                    ...
                    if "get_branch_recent_commits_tool" in tools_dict:
                        commits_tool = tools_dict["get_branch_recent_commits_tool"]
                        logger.info(
                            f"📜 Checking recent commits in {repo_name} repository"
                        )
                        ...
                        if commits_result:
                            ...
                            logger.info(
                                f"✅ Commits for {service} from {successful_branch}: {len(str(commits_result))} chars"
                            )
                        else:
                            error_info = parse_tool_error(
                                "",
                            ...
                            logger.warning(
                                f"⚠️ {error_info['user_message']} (tried: {', '.join(attempted_branches)})"
                            )
                            commit_findings[service] = (
                                f"Could not fetch commits (tried branches: {', '.join(attempted_branches)})"
                            )
                    else:
                        logger.warning(
                            "⚠️ No commit fetching tool available (need GitHub integration)"
                        )
                        commit_findings[service] = (
                            "Commit tool not available - GitHub integration required"
                        )

                except Exception as e:
                    error_info = parse_tool_error(
                        str(e), "get_branch_recent_commits_tool", f"service '{service}'"
                    )
                    logger.error(
                        f"Error fetching commits for {service}: {error_info['user_message']}"
                    )
        ...
        telemetry = {
            ...
            "tool_availability": {
                ...
                "code_tools": "read_repository_file_tool" in tools_dict
                and "get_repository_tree_tool" in tools_dict,
        ...
        if "logs" in tools_to_use and not logs_summary:
            if "search_newrelic_logs_tool" not in tools_dict:
                telemetry["skipped_tools"].append(
                    {"tool": "logs", "reason": "tool_not_available"}
                )
            else:
                telemetry["skipped_tools"].append(
                    {"tool": "logs", "reason": "collection_failed"}
                )

        if "metrics" in tools_to_use and not metrics_summary:
            if "query_newrelic_metrics_tool" not in tools_dict:
                telemetry["skipped_tools"].append(
                    {"tool": "metrics", "reason": "tool_not_available"}
                )
            else:
                telemetry["skipped_tools"].append(
                    {"tool": "metrics", "reason": "collection_failed"}
                )

        if "code" in tools_to_use and not code_findings:
            if "read_repository_file_tool" not in tools_dict:
                telemetry["skipped_tools"].append(
                    {"tool": "code", "reason": "tool_not_available"}
                )
            else:
                telemetry["skipped_tools"].append(
                    {"tool": "code", "reason": "files_not_accessible"}
                )

        if "commits" in tools_to_use and not commit_findings:
            if "get_branch_recent_commits_tool" not in tools_dict:
                telemetry["skipped_tools"].append(
                    {"tool": "commits", "reason": "tool_not_available"}
                )
            else:
                telemetry["skipped_tools"].append(
                    {"tool": "commits", "reason": "branch_not_found"}
                )
        ...
        logger.info(
            f"Investigation telemetry: {telemetry['data_collected']} data points collected"
        )
...
async def iterative_investigate_node(
    ...
    try:
        ...
        for depth in range(max_depth):
            logger.info(f"\n{'=' * 60}")
            logger.info(
                f"🔍 INVESTIGATION LEVEL {depth + 1}/{max_depth}: `{current_service}`"
            )
            logger.info(f"{'=' * 60}\n")
            ...
            if current_service not in service_mapping and current_service != "unknown":
                logger.warning(
                    f"⚠️ Service '{current_service}' not in mapping. Available: {list(service_mapping.keys())}"
                )
                # Try fuzzy matching
                best_match = None
                best_score = 0
                for mapped_service in service_mapping.keys():
                    score = SequenceMatcher(
                        None, current_service.lower(), mapped_service.lower()
                    ).ratio()
                    if score > best_score:
                        best_score = score
                ...
                if best_match and best_score > 0.6:
                    logger.info(
                        f"   Fuzzy matched '{current_service}' → '{best_match}' (score: {best_score:.2f})"
                    )
                    current_service = best_match
                else:
                    logger.error(
                        f"   Could not map service '{current_service}' to any repository. Stopping investigation."
                    )
                    break
            ...
            if decision["decision"] == "ROOT_CAUSE_FOUND":
                ...
                if not upstream_services:
                    ...
                    logger.info(
                        "   LLM suggested upstream investigation but didn't specify services. Extracting from findings..."
                    )
                    files = state.get("files", [])
                    upstream_services = await extract_upstream_dependencies(
                        findings, llm, files=files
                    )

                if not upstream_services:
                    logger.warning(
                        "   No upstream services identified from LLM or extraction."
                    )
                ...
                if upstream_services:
                    ...
                    if any(
                        item["service"] == next_service for item in investigation_chain
                    ):
                        logger.warning(
                            f"   ⚠️ Cycle detected! Already investigated `{next_service}`. Stopping."
                        )
                    ...
                    logger.info(
                        "   🔄 Using fallback: checking all uninvestigated services"
                    )
                    ...
                    uninvestigated = [
                        ...
                        and not any(
                            item["service"] == s for item in investigation_chain
                        )
                    ]
                    ...
                    if uninvestigated:
                        ...
                        logger.info(
                            f"   🔄 Fallback → Investigating `{next_service}` (exhaustive search)"
                        )
                        current_service = next_service
                        ...
                        logger.warning(
                            "   No upstream found and all services checked. Investigation inconclusive."
                        )
                ...
                logger.warning(
                    f"   LLM decision: INCONCLUSIVE (confidence: {decision['confidence']}%)"
                )
                logger.info(
                    "   🔄 Using fallback strategy: checking all uninvestigated services"
                )
                ...
                if uninvestigated:
                    ...
                    # Truly exhausted all services
                    logger.info(
                        f"   ✅ Checked all {len(investigation_chain)} services, no clear root cause found"
                    )
                    final_decision = "INCONCLUSIVE"
                    break
        else:
            ...
            logger.warning(
                f"⚠️ Reached maximum investigation depth ({max_depth}). Stopping."
            )
        ...
        logger.info(
            f"   Services traced: {[item['service'] for item in investigation_chain]}"
        )
        ...
        intermediate_steps.append(
            {
                ...
                "result": {
                    ...
                    "services_traced": [
                        item["service"] for item in investigation_chain
                    ],
...
async def analyze_and_trace_node(
    state: RCAStateV2, llm: BaseChatModel
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if investigation_chain:
            logger.info(
                f"🔗 Multi-level investigation detected ({len(investigation_chain)} services traced)"
            )

            # Extract victim, intermediate, and root services
            victim_service = (
                investigation_chain[0]["service"]
                if investigation_chain
                else primary_service
            )
            intermediate_services = (
                [item["service"] for item in investigation_chain[1:-1]]
                if len(investigation_chain) > 2
                else []
            )
            root_service_candidate = (
                investigation_chain[-1]["service"] if investigation_chain else "unknown"
            )
            ...
            last_logs = last_findings.get("logs_summary", {}).get(
                root_service_candidate, "No logs"
            )
            last_metrics = last_findings.get("metrics_summary", {}).get(
                root_service_candidate, "No metrics"
            )
            last_code = last_findings.get("code_findings", {}).get(
                root_service_candidate, "No code"
            )
            last_commits = last_findings.get("commit_findings", {}).get(
                root_service_candidate, "No commits"
            )
...
Respond now:"""
            ...
            # Parse response
            root_service = (
                extract_field(content, "ROOT_SERVICE") or root_service_candidate
            )
            ...
            logger.info("✅ Multi-level RCA complete:")
            ...
            return {
                ...
                if root_commit and root_commit != "none"
                else None,
                "confidence": confidence,
...
async def generate_report_node(state: RCAStateV2, llm: BaseChatModel) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not is_incident:
            ...
            intermediate_str = (
                ", ".join([f"`{s}`" for s in intermediate_services])
                if intermediate_services
                else "None"
            )
            ...
            prompt = GENERATE_INCIDENT_PROMPT_V1.format(
                ...
                recent_commits=format_dict_for_context(
                    commit_findings, max_length=1000
                ),
            )
