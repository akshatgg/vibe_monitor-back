...
class QueryLogsRequest(BaseModel):
    ...
    nrql_query: str = Field(..., description="NRQL query string for logs (e.g., 'SELECT * FROM Log WHERE ...')")
...
class QueryLogsResponse(BaseModel):
    ...
    results: List[Dict[str, Any]] = Field(..., description="Query results as list of log entries")
    totalCount: int = Field(..., description="Total number of results returned")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Query metadata (e.g., performance stats)")
...
class FilterLogsRequest(BaseModel):
    ...
    limit: Optional[int] = Field(100, description="Maximum number of results (default: 100)", ge=1, le=1000)
    offset: Optional[int] = Field(0, description="Offset for pagination (default: 0)", ge=0)
