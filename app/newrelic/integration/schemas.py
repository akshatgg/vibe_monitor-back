...
class NewRelicIntegrationCreate(BaseModel):
    ...
    api_key: str = Field(
        ..., description="New Relic User API Key (must start with NRAK)"
    )
...
class NewRelicIntegrationStatusResponse(BaseModel):
    ...
    is_connected: bool = Field(
        ..., description="Whether the workspace is connected to New Relic"
    )
    integration: Optional[NewRelicIntegrationResponse] = Field(
