...
class QueryMetricsRequest(BaseModel):
    ...
    nrql_query: str = Field(..., description="NRQL query string for metrics (e.g., 'SELECT average(duration) FROM Transaction ...')")
...
class QueryMetricsResponse(BaseModel):
    ...
    results: List[Dict[str, Any]] = Field(..., description="Query results as list of metric data points")
...
class GetTimeSeriesRequest(BaseModel):
    ...
    metric_name: str = Field(..., description="Metric name (e.g., 'apm.service.transaction.duration')")
    ...
    aggregation: Optional[str] = Field("average", description="Aggregation function: average, sum, min, max, count")
    timeseries: Optional[bool] = Field(True, description="Return as time series (default: true)")
...
class TimeSeriesDataPoint(BaseModel):
    ...
    value: Optional[float] = Field(None, description="Metric value (can be None if no data)")
...
class GetTimeSeriesResponse(BaseModel):
    ...
    dataPoints: List[TimeSeriesDataPoint] = Field(..., description="Time series data points")
...
class GetInfraMetricsRequest(BaseModel):
    ...
    metric_name: str = Field(..., description="Infrastructure metric name (e.g., 'cpuPercent', 'memoryUsedPercent')")
    hostname: Optional[str] = Field(None, description="Filter by hostname")
    startTime: int = Field(..., description="Start time in seconds since epoch")
    endTime: int = Field(..., description="End time in seconds since epoch")
    aggregation: Optional[str] = Field("average", description="Aggregation function: average, sum, min, max")
