...
def upgrade() -> None:
    ...
    sa.Column('event_type', sa.Enum('PROMPT_INJECTION', 'GUARD_DEGRADED', name='securityeventtype'), nullable=False),
    ...
    sa.ForeignKeyConstraint(['slack_integration_id'], ['slack_installations.id'], ),
    sa.ForeignKeyConstraint(['workspace_id'], ['workspaces.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_security_events_detected_at', 'security_events', ['detected_at'], unique=False)
    op.create_index('idx_security_events_slack_integration', 'security_events', ['slack_integration_id'], unique=False)
    op.create_index('idx_security_events_slack_user', 'security_events', ['slack_user_id'], unique=False)
    op.create_index('idx_security_events_type', 'security_events', ['event_type'], unique=False)
    op.create_index('idx_security_events_workspace', 'security_events', ['workspace_id'], unique=False)
    op.drop_index(op.f('idx_newrelic_integration_workspace'), table_name='newrelic_integrations')
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_index(op.f('idx_newrelic_integration_workspace'), 'newrelic_integrations', ['workspace_id'], unique=False)
