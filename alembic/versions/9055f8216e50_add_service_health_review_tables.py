"""add_service_health_review_and_code_parser_tables
...
def upgrade() -> None:
    ...
    # Code Parser tables
    op.create_table('parsed_repositories',
    sa.Column('id', sa.String(), nullable=False),
    sa.Column('workspace_id', sa.String(), nullable=False),
    sa.Column('repo_full_name', sa.String(length=255), nullable=False),
    sa.Column('default_branch', sa.String(length=255), nullable=True),
    sa.Column('commit_sha', sa.String(length=40), nullable=False),
    sa.Column('status', sa.Enum('PENDING', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'SKIPPED', name='parsingstatus'), nullable=False),
    sa.Column('error_message', sa.Text(), nullable=True),
    ...
    sa.Column('parse_errors', postgresql.JSON(astext_type=sa.Text()), nullable=True),
    ...
    sa.ForeignKeyConstraint(['workspace_id'], ['workspaces.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('workspace_id', 'repo_full_name', 'commit_sha', name='uq_parsed_repo_workspace_repo_commit')
    )
    op.create_index('idx_parsed_repositories_commit', 'parsed_repositories', ['commit_sha'], unique=False)
    op.create_index('idx_parsed_repositories_repo_name', 'parsed_repositories', ['repo_full_name'], unique=False)
    op.create_index('idx_parsed_repositories_status', 'parsed_repositories', ['status'], unique=False)
    op.create_index('idx_parsed_repositories_workspace', 'parsed_repositories', ['workspace_id'], unique=False)
    ...
    sa.Column('functions', postgresql.JSON(astext_type=sa.Text()), nullable=True),
    sa.Column('classes', postgresql.JSON(astext_type=sa.Text()), nullable=True),
    sa.Column('imports', postgresql.JSON(astext_type=sa.Text()), nullable=True),
    sa.Column('is_parsed', sa.Boolean(), nullable=False),
    sa.Column('parse_error', sa.Text(), nullable=True),
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.ForeignKeyConstraint(['repository_id'], ['parsed_repositories.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('idx_parsed_files_language', 'parsed_files', ['language'], unique=False)
    op.create_index('idx_parsed_files_path', 'parsed_files', ['file_path'], unique=False)
    op.create_index('idx_parsed_files_repository', 'parsed_files', ['repository_id'], unique=False)
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    # Drop Code Parser tables first (reverse order of creation)
    op.drop_index('idx_parsed_files_repository', table_name='parsed_files')
    op.drop_index('idx_parsed_files_path', table_name='parsed_files')
    op.drop_index('idx_parsed_files_language', table_name='parsed_files')
    op.drop_table('parsed_files')
    op.drop_index('idx_parsed_repositories_workspace', table_name='parsed_repositories')
    op.drop_index('idx_parsed_repositories_status', table_name='parsed_repositories')
    op.drop_index('idx_parsed_repositories_repo_name', table_name='parsed_repositories')
    op.drop_index('idx_parsed_repositories_commit', table_name='parsed_repositories')
    op.drop_table('parsed_repositories')
