...
def upgrade() -> None:
    """Upgrade schema."""
    # If integrations table exists with integration_id columns populated,
    # this migration was already applied
    if table_exists("integrations") and column_exists(
        "github_integrations", "integration_id"
    ):
        return
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "status IN ('active', 'disabled', 'error')", name="check_integration_status"
        ),
        sa.CheckConstraint(
            "health_status IS NULL OR health_status IN ('healthy', 'degraded', 'failed', 'unknown')",
            name="check_integration_health_status",
        ),
        sa.UniqueConstraint(
    ...
    op.create_index(
        "idx_integrations_workspace_provider",
        "integrations",
    ...
    for table in [
        ...
        op.create_unique_constraint(
            f"uq_{table}_integration_id", table, ["integration_id"]
        )
    ...
    op.create_unique_constraint(
        "uq_slack_installations_integration_id",
        "slack_installations",
        ["integration_id"],
    )
    op.create_index(
        "idx_slack_installations_integration_id",
        "slack_installations",
        ["integration_id"],
    )
