...
def main():
    ...
    print("Instructions:")
    print(
        "1. Go to: http://localhost:8000/api/v1/auth/login?redirect_uri=http://localhost:8000/"
    )
    ...
    while True:
        try:
            ...
            if not redirect_url.startswith("http"):
                print(
                    "❌ Invalid URL format. Please paste the full URL starting with http"
                )
                continue
            ...
            if not code:
                print(
                    "❌ Could not extract OAuth code. Please try again with a fresh login."
                )
                continue
            ...
            print("\n" + "=" * 50)
            print(
                "Want to test again? Get a new login URL and paste it, or Ctrl+C to exit"
            )
            print()
